  ...
  return (
    <div className="flex h-screen bg-[#050608] text-white overflow-hidden">
      ...
      <aside className="flex w-[260px] flex-col border-r border-white/5 bg-[#07090e] px-6 py-8">
        <div className="mb-12">
          <h1 className="text-[28px] font-black leading-none tracking-tighter text-brand">
            ZOONLABS
          </h1>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.id}
                href={item.href}
                className={`group flex items-center gap-4 rounded-xl px-4 py-3.5 text-sm font-bold transition-all ${
                  active ? "bg-white/5 text-brand" : "text-[#8b909c] hover:bg-white/[0.02] hover:text-white"
                }`}
              >
