...
export function FileList({ files, onDownload, onDelete }: FileListProps) {
  const getFileIcon = (kind: string) => {
    const iconProps = { className: "size-6 text-brand", strokeWidth: 1.5 };
    switch (kind) {
      case "PDF":
        return (
            ...
            <span className="text-sm font-semibold text-mute">{file.size}</span>
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-2 text-zinc-400 hover:text-white hover:bg-white/5 transition-all rounded-md"
                  ...
                  <button
                    ...
                    className="p-2 text-zinc-400 hover:text-brand hover:bg-brand/5 transition-all rounded-md"
                    title="Purge Asset"
                  >
